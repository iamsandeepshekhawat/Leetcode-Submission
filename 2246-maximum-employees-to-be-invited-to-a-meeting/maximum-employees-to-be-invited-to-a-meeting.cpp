...
public:
    int maximumInvitations(vector<int>& adj) {
        ...
        // Step1 Make reverse adjacency list and count Indegree
        for(int i=0;i<n;i++){
        ...
        // Step2 Mark nodes not in any cycle (using topological sort)
        int tot_tail_len = 0;
        vector<bool> visited(n,false);
        for(int i=0;i<n;i++){
        ...
        // Step3 For Cycle-size >2, track max size across all cycles
        ...
        // Step4 Find the best option of the 2 cases and return (All 2 size cycles can be added)
